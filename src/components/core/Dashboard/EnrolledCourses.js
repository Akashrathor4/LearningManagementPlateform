            ...
            >
              <div
                ...
                onClick={() => {
                  ...
                  // )
                  console.log("Course of Enrolled ",course);
                  console.log("courseContent of Enrolled", course.courseContent);

                  const sectionId = course.courseContent?.[0]?._id;
                  console.log("sectionId of Enrolled ",sectionId);
                  const subSectionId = course.courseContent?.[0]?.subSection?.[0]?._id;
                  console.log("subSectionId of Enrolled ",subSectionId);

                  if (sectionId && subSectionId) {
                    navigate(`/view-course/${course._id}/section/${sectionId}/sub-section/${subSectionId}`);
                  } else {
                    console.warn("Missing section or subsection ID for course:", course);
                    // Optional: redirect to just the course page or show a warning
